...
class NewWorkflowService:
    """Service for the new integrated skill-based interview workflow."""
    ...
    async def start_interview(self, record_id: str) -> Dict[str, Any]:
        ...
        logger.info(f"Starting new workflow interview for record_id: {record_id}")
    ...
    async def submit_answer(self, interview_id: str, answer: str) -> Dict[str, Any]:
        ...
        if session["current_question_index"] >= len(session["questions"]):
            ...
            analysis = await self._generate_interview_analysis(session)
            session["analysis"] = analysis

            # Create Opportunity Discussed record in Salesforce
            try:
                self._salesforce_client.create_opportunity_discussed_record(
                    candidate_id=session["record_id"],
                    screening_transcript=transcript,
                    ai_interview_summary=analysis,
                )
            except Exception as e:
                logger.error(f"Failed to create Opportunity Discussed record: {str(e)}")
    ...
    async def _generate_transcript(self, session: Dict[str, Any]) -> str:
        ...
        transcript_lines = []
        transcript_lines.append("NEW WORKFLOW INTERVIEW TRANSCRIPT")
        transcript_lines.append("=" * 50)
        transcript_lines.append(f"Candidate ID: {session['record_id']}")
        transcript_lines.append(f"Interview Date: {uuid.uuid4().hex[:8]}")
    ...
    async def _generate_interview_analysis(self, session: Dict[str, Any]) -> str:
        """Generate an AI analysis of the interview."""
        prompt = (
            "You are an expert technical recruiter providing an analysis of a candidate's interview. "
            "Based on the interview transcript, provide a concise, insightful analysis of the candidate's skills, "
            "communication, and overall suitability for a technical role.\n\n"
            "Focus on:\n"
            "- Clarity and depth of answers\n"
            "- Evidence of practical application of skills\n"
        ...
        try:
            response = await self._client.chat.completions.create(
                model=self._model,
                messages=[
                    {"role": "system", "content": "You are a thoughtful and insightful interview analyst."},
                    {"role": "user", "content": prompt},
                ],
            )
            return response.choices[0].message.content or ""
        except Exception as e:
            logger.error(f"Failed to generate interview analysis: {str(e)}")
